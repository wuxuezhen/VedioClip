...
- (void)resetSubviews{
    ...
    CGFloat ratio = self.showsRulerView ? 0.7 : 0.8;
    ...
    if (self.showsRulerView) {
        CGRect rulerFrame = CGRectMake(0
                                       , CGRectGetHeight(self.contentView.frame)*0.7
                                       , CGRectGetWidth(self.contentView.frame)+self.thumbWidth
                                       , CGRectGetHeight(self.contentView.frame)*0.3);
        
        ICGRulerView *rulerView = [[ICGRulerView alloc] initWithFrame:rulerFrame
                                                       widthPerSecond:self.widthPerSecond
                                                           themeColor:self.themeColor
                                                        labelInterval:self.rulerLabelInterval];
    ...
    self.leftOverlayView = [[HitTestView alloc] initWithFrame:CGRectMake(self.thumbWidth - self.overlayWidth
                                                                         ,0
                                                                         ,self.overlayWidth
                                                                         ,CGRectGetHeight(self.frameView.frame))];
    self.leftOverlayView.hitTestEdgeInsets = UIEdgeInsetsMake(0, 0, 0, -(EDGE_EXTENSION_FOR_THUMB));
    
    CGRect leftThumbFrame = CGRectMake(self.overlayWidth-self.thumbWidth
                                       , 0
                                       , self.thumbWidth
                                       , CGRectGetHeight(self.frameView.frame));
    
    if (self.leftThumbImage) {
        self.leftThumbView = [[ICGThumbView alloc] initWithFrame:leftThumbFrame
                                                      thumbImage:self.leftThumbImage];
    } else {
        self.leftThumbView = [[ICGThumbView alloc] initWithFrame:leftThumbFrame
    ...
    self.startImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, CGRectGetHeight(self.frameView.frame) - 5, 10, 10)];
    self.startImageView.backgroundColor = [UIColor yellowColor];
    [self addSubview:self.startImageView];
    
    
    self.trackerView = [[UIView alloc] initWithFrame:CGRectMake(self.thumbWidth
                                                                , 0
                                                                , 1
                                                                , CGRectGetHeight(self.frameView.frame))];
    ...
    CGRect rightThumbFrame = CGRectMake(0, 0, self.thumbWidth, CGRectGetHeight(self.frameView.frame));
    if (self.rightThumbImage) {
        self.rightThumbView = [[ICGThumbView alloc] initWithFrame:rightThumbFrame
                                                       thumbImage:self.rightThumbImage];
    } else {
        self.rightThumbView = [[ICGThumbView alloc] initWithFrame:rightThumbFrame
                                                            color:self.themeColor right:YES];
    }
    
    self.endImageView = [[UIImageView alloc]initWithFrame:CGRectMake(CGRectGetWidth(self.frame) -5, CGRectGetHeight(self.frameView.frame), 10, 10)];
    self.endImageView.backgroundColor = [UIColor yellowColor];
...
- (void)updateBorderFrames{
    ...
    CGFloat width  = CGRectGetMinX(self.rightOverlayView.frame)-CGRectGetMaxX(self.leftOverlayView.frame);
    [self.topBorder setFrame:CGRectMake(CGRectGetMaxX(self.leftOverlayView.frame), 0, width, height)];
    [self.bottomBorder setFrame:CGRectMake(CGRectGetMaxX(self.leftOverlayView.frame), CGRectGetHeight(self.frameView.frame)-height, width, height)];
    
    self.startImageView.frame = CGRectMake(CGRectGetMaxX(self.leftOverlayView.frame) - 5, CGRectGetHeight(self.frameView.frame), 10, 10);
    self.endImageView.frame = CGRectMake(CGRectGetMaxX(self.leftOverlayView.frame) + width - 5, CGRectGetHeight(self.frameView.frame), 10, 10);
    
    [self drawDashLine:self.topBorder lineLength:5 lineSpacing:5 lineColor:[UIColor yellowColor]];
    [self drawDashLine:self.bottomBorder lineLength:5 lineSpacing:5 lineColor:[UIColor yellowColor]];
}
